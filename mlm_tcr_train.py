...
aa_dict = mk_aa_dict()
lm_config = {
    'max_len':20,'d_seq':128, 'd_pair':64,
...
train_config = {
    ...
    'lr':5e-5, 'weight_decay':0.01, 'max_epoch':500, 
     'patience':50, 'save_path':'cdr3_mlm.pt'
}
...
for epoch in t: 
    ...
    train_dataloader = DataLoader(MaskedLMDataSet(
        seq_list=epoch_df_train['cdr3'].tolist(), 
    ...
    valid_dataloader = DataLoader(MaskedLMDataSet(
        seq_list=epoch_df_valid['cdr3'].tolist(), 
        max_len=lm_config['max_len'], masked_rate=train_config['masked_rate'], 
        contiguous_prob=train_config['contiguous_prob']
...
with rc_context({'figure.figsize': (5, 4)}):
    ...
    plt.title("Training & Validation Loss for CDR3 LM")
    plt.xlabel("Epochs")
    plt.ylabel("Loss")
    plt.legend()
    plt.xticks(size = 14)
    plt.yticks(size = 14)
    plt.savefig('cdr3_loss_curve.png', dpi=200, bbox_inches='tight')
    plt.savefig('cdr3_loss_curve.pdf',bbox_inches='tight')
    plt.close()
...
}).to_csv("cdr3_loss_curve.csv", index=False)
