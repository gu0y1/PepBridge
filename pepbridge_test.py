...
def model_inference(model, dataloader, task, device):
    ...
    with torch.no_grad():
        for b, idx in dataloader:
            if task in ('mp','mp_contact','imm'):
                ...
                out = model.mp_pred(mhc, peptide, esm_mhc, 
                                    contact= (task == 'mp_contact'), 
                                    immunogenicity= (task == 'imm'))
                mask_1 = out["mask_dict"]["mhc"]
                mask_2 = out["mask_dict"]["pep"]

            elif task in ('pt','pt_contact'):
                peptide = b["peptide"].to(device)
                cdr3    = b["cdr3"].to(device)
                out = model.pt_pred(peptide, cdr3, 
                                    contact=(task == 'pt_contact'))
                mask_1 = out["mask_dict"]["pep"]
                mask_2 = out["mask_dict"]["cdr3"]
                ...
                out = model.mpt_pred(mhc, peptide, cdr3, esm_mhc, trbv)
            else:
            ...
            if task in ('mp_contact', 'pt_contact'):
                ...
                pred_dist[idx] = out['contact_dist'].detach().cpu().numpy()
                pred_prob[idx] = torch.sigmoid(out['contact_prob'].detach().cpu()).numpy()

            elif task in ('mp', 'pt'):
                pred = torch.sigmoid(out['binding_prob'].mean(dim=1, keepdim=True).detach().cpu()).numpy()
                arr_prob[idx] = pred

            elif task == 'imm':
                pred = torch.sigmoid(out['immunogenicity_prob'].mean(dim=1, keepdim=True).detach().cpu()).numpy()
                arr_prob[idx] = pred

            elif task == 'mpt':
                pred = torch.sigmoid(out['mpt_prob'].mean(dim=1, keepdim=True).detach().cpu()).numpy()
                arr_prob[idx] = pred

    if task in ('mp_contact', 'pt_contact'):
        out_dict = {}
        out_dict['mask'] = arr_mask
        out_dict['gt_prob'] = gt_prob
        out_dict['gt_dist'] = gt_dist
        out_dict['pred_dist'] = pred_dist
        out_dict['pred_prob'] = pred_prob
        return out_dict 
    else:
        return arr_prob
...
if __name__ == "__main__":
    ...
    path = cli_args.get("path", default_ckpt_path)
    ...
    logger.info(binary_evaluate_metrics(pt_df['binding'],pt_df['pred'],group=pt_df['peptide'],mean=True))


    logger.info('mpt_binding:')
    mpt_df['pred'] = model_inference(model, mpt_loader, 'mpt', device)
    ...
    logger.info('avg pre-peptide:')
    logger.info(binary_evaluate_metrics(mpt_df['binding'],mpt_df['pred'],group=mpt_df['peptide'],mean=True))


    logger.info('imm:')
    imm_df['pred'] = model_inference(model, imm_loader, 'imm', device)
    logger.info(binary_evaluate_metrics(imm_df['immunogenicity'], imm_df['pred']))

    logger.info('mp_contact:')
    mp_contact_out = model_inference(model, mp_contact_loader, 'mp_contact', device)
