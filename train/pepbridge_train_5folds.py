...
from script.dataset import PTDataSet, MPDataSet, MPTDataSet, build_loader_uniform_by_peptide
...
if __name__ == "__main__":
    ...
    default_ckpt_path = os.path.join(BASE, "checkpoints_multi_lora_align_all/")
    ...
    mpt_df = pd.read_csv(os.path.join(DATA,'mpt_train.csv'), header=0, index_col=0)
    align_train = pd.read_csv(os.path.join(DATA,'random_pMHC_cdr3_train.csv'), header=0, index_col=0)
    align_val = pd.read_csv(os.path.join(DATA,'random_pMHC_cdr3_test.csv'), header=0, index_col=0)
    imm_df = pd.read_csv(os.path.join(DATA,'immunogenicity_train.csv'), header=0, index_col=0)
    mp_contact_df = pd.read_csv(os.path.join(CODE,'data/mp_pdb_train.csv'),  header=0, index_col=0)
    pt_contact_df = pd.read_csv(os.path.join(CODE,'data/pt_pdb_train.csv'), header=0, index_col=0)
    ...
    for i in range(n_folds): 
        save_dir = os.path.join(save_root, f"fold_{i+1}")
        ...
        pt_train, pt_val= df_train_test_split(pt_df, seed=seed, n_splits=n_folds, fold=i)
        ...
        model = encoder_load_state_dict(model, peptide_pt_path=os.path.join(CODE,'doc/peptide_mlm.pt'),
                                        cdr3_pt_path=os.path.join(CODE, 'doc/cdr3_mlm.pt'), 
                                        device=device)
        ...
        train_three_phases_multi_loaders(  
            ...
            log_interval=200,
            task_every = {"mp_contact": 20, "pt_contact": 25},   #50
