...
if __name__ == "__main__":
    ...
    mp_contact_df = pd.read_csv(os.path.join(DATA,'mp_pdb_train.csv'),  header=0, index_col=0)
    pt_contact_df = pd.read_csv(os.path.join(DATA,'pt_pdb_train.csv'), header=0, index_col=0)
    ...
    for i in range(n_folds): 
        ...
        model = encoder_load_state_dict(model, peptide_pt_path=os.path.join(BASE,'doc/peptide_mlm.pt'),
                                        cdr3_pt_path=os.path.join(BASE, 'doc/cdr3_mlm.pt'), 
                                        device=device)
        ...
        train_three_phases_multi_loaders(  
            ...
            log_interval=200,
            task_every = {"mp_contact": 20, "pt_contact": 25},   
