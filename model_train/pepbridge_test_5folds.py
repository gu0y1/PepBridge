...
if __name__ == "__main__":
    ...
    #    
    pt_hybrid_df = pd.read_csv(os.path.join(DATA,'pt_test_hybrid.csv'), header=0, index_col=0).reset_index(drop=True)
...
####
    ...
    pt_hybrid_loader = DataLoader(
        dataset=PTDataSet(pt_hybrid_df, pep_max_len=15, cdr3_max_len=20,
    ...
    logger.info('pt_hybrid_binding:')
    pt_hybrid_df['pred'] = model_inference(models, pt_hybrid_loader, 'pt', device)
    logger.info(binary_evaluate_metrics(pt_hybrid_df['binding'],pt_hybrid_df['pred']))
    logger.info(binary_evaluate_metrics(pt_hybrid_df['binding'],pt_hybrid_df['pred'],
                                        group=pt_hybrid_df['peptide_category']))
    ...
    logger.info('pt_contact_2022:')                                    
    pt_contact_out = model_inference(models, pt_contact_loader_2022_2026, 'pt_contact', device)
    logger.info(binary_evaluate_metrics(pt_contact_out['gt_prob'], 
                                        pt_contact_out['pred_prob'], 
                                        pt_contact_out['mask']))
    logger.info(distance_evaluate_metrics(pt_contact_out['gt_dist'], 
